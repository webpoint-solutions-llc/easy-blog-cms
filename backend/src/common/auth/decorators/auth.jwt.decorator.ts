...
import { AuthPayloadRevokeUser } from '../guards/payload/auth.payload.revoke-user.guard';
...
export function AuthRevokeInactiveUserGuard() {
    return applyDecorators(
        UseGuards(JwtGuard, AuthPayloadDefaultGuard, AuthPayloadRevokeUser)
    );
}
